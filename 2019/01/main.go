...
func main() {
	...
	go partOne(inputs, &wg)
	go partTwo(inputs, &wg)
...
func partOne(inputs []string, wg *sync.WaitGroup) {
	defer wg.Done()
	fmt.Printf("part one result: %v\n", calculate(inputs, calculateFuel))
}
...
func partTwo(inputs []string, wg *sync.WaitGroup) {
	defer wg.Done()
...
func calculateWholeFuel(f float64) int {

	var sum int
	for {
		residue := calculateFuel(f)
...
func calculateFuel(f float64) int { return int(f/3) - 2 }
