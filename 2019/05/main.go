...
type opcodeFunc func([]int, ...int) (bool, error)

var opcodes = map[opcode]opcodeFunc{
	ocOne: func(arr []int, args ...int) (bool, error) {
		if len(args) != 3 {
	...
	ocTwo: func(arr []int, args ...int) (bool, error) {
		if len(args) != 3 {
			return false, errInvalidArgsNum
		}
	...
	ocThree: func(arr []int, args ...int) (bool, error) {
		if len(args) != 1 {
			return false, errInvalidArgsNum
		}
	...
	ocFour: func(arr []int, args ...int) (bool, error) {
		if len(args) != 1 {
			return false, errInvalidArgsNum
		}
		output = arr[args[0]]
	...
	oc99: func(arr []int, args ...int) (bool, error) {
		return true, nil
	},
}
...
func modeOne(arr []int, ocInput int, args ...int) (bool, error) {
...
func main() {
	...
	for {
		...
		ocf, ok := opcodes[oc]
		if !ok {
			ocf = func(arr []int, args ...int) (bool, error) {
				return modeOne(arr, int(oc), args...)
			}
		}

		finished, err := ocf(arr, args...)
		if err != nil {
			panic(err)
		}
